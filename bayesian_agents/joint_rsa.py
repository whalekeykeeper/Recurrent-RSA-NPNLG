...
class RSA:
    ...
    def initialize_speakers(self, paths):
        self.initial_speakers = [
            Model(path=path, dictionaries=(self.seg2idx, self.idx2seg))
            for path in paths
        ]
        self.speaker_prior = Model(
            path="lang_mod", dictionaries=(self.seg2idx, self.idx2seg)
        )
    ...
    def speaker(self, state, world, depth):
        ...
        if depth == 0:
            ...
            return self.initial_speakers[world.speaker].forward(
        ...
        if depth == 1:
            ...
            scores = scores * (
                self.initial_speakers[world.speaker].rationality_support[
                    world.rationality
                ]
            )
            # update prior to posterior
            # print(scores.shape,prior.shape)
            posterior = (scores + prior) - logsumexp(scores + prior)
    ...
    def listener(self, state, utterance, depth):
        ...
        for n_tuple in itertools.product(*[list(range(x)) for x in world_prior.shape]):
            ...
            # world.target=j
            world = RSA_World(
                target=n_tuple[state.dim["image"]],
        ...
        world_posterior = (scores + world_prior) - logsumexp(scores + world_prior)
